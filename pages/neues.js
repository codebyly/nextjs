...
import NeuesListe from "../components/NeuesListe";
...
export async function getStaticProps() {
  ...
  //     `https://newsapi.org/v2/top-headlines?apiKey=${apiKey}&country=de&category=entertainment&pageSize=10`
  //   );
...
export default function News({ news, time, test }) {
  return (
    <Layout title="News">
      <p>{test}</p>
      <p>heute: {time}</p>
      <NeuesListe news={news} title="Neues aus aller Welt" />
    </Layout>
  );
}
